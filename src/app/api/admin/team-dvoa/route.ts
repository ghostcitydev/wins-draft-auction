import { NextRequest, NextResponse } from "next/server";
import { eq } from "drizzle-orm";
import { db } from "@/db";
import { teams, teamPlayoffOdds, appConfig } from "@/db/schema";
import { parseFtnDvoaPaste } from "@/lib/ftn-dvoa-parser";

export const dynamic = "force-dynamic";

function checkAuth(req: NextRequest) {
  const secret = process.env.ADMIN_SECRET;
  if (!secret) return true;
  return req.headers.get("x-admin-secret") === secret;
}

interface PastePayload {
  season: number;
  week: number;
  rawText: string;
}

export async function POST(req: NextRequest) {
  if (!checkAuth(req)) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const body: PastePayload = await req.json();
  if (!body.rawText || !body.season || !body.week) {
    return NextResponse.json({ error: "Expected { season, week, rawText }" }, { status: 400 });
  }

  const { rows, errors } = parseFtnDvoaPaste(body.rawText);

  const allTeams = await db.select().from(teams);
  const byAbbr = new Map(allTeams.map((t) => [t.abbr, t]));

  let upserted = 0;
  const unmatchedAbbrs: string[] = [];

  for (const row of rows) {
    const team = byAbbr.get(row.abbr);
    if (!team) {
      unmatchedAbbrs.push(row.abbr);
      continue;
    }

    await db
      .insert(teamPlayoffOdds)
      .values({
        season: body.season,
        week: body.week,
        teamId: team.id,
        source: "ftn",
        wins: row.wins,
        losses: row.losses,
        dave: row.dave,
        meanWins: row.meanWins,
        tot: row.tot,
        div: row.div,
        wc: row.wc,
        seed1: row.seed1,
        seed2: row.seed2,
        seed3: row.seed3,
        seed4: row.seed4,
        seed5: row.seed5,
        seed6: row.seed6,
        seed7: row.seed7,
      })
      .onConflictDoUpdate({
        target: [teamPlayoffOdds.season, teamPlayoffOdds.week, teamPlayoffOdds.teamId],
        set: {
          wins: row.wins,
          losses: row.losses,
          dave: row.dave,
          meanWins: row.meanWins,
          tot: row.tot,
          div: row.div,
          wc: row.wc,
          seed1: row.seed1,
          seed2: row.seed2,
          seed3: row.seed3,
          seed4: row.seed4,
          seed5: row.seed5,
          seed6: row.seed6,
          seed7: row.seed7,
        },
      });
    upserted++;
  }

  return NextResponse.json({
    upserted,
    totalParsed: rows.length,
    parseErrors: errors,
    unmatchedAbbrs,
  });
}

export async function GET(req: NextRequest) {
  if (!checkAuth(req)) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const config = await db.select().from(appConfig).where(eq(appConfig.id, "singleton"));
  const season = config[0]?.season ?? 2026;
  const rows = await db.select().from(teamPlayoffOdds).where(eq(teamPlayoffOdds.season, season));
  const weeks = [...new Set(rows.map((r) => r.week))].sort((a, b) => b - a);
  return NextResponse.json({ season, weeksLogged: weeks, rowCount: rows.length });
}
