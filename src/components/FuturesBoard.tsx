      ...
      <table className="w-full min-w-[480px] border-collapse text-xs">
        <thead>
          <tr className="border-b border-border">
            <th className="sticky left-0 z-10 w-28 min-w-[110px] max-w-[110px] bg-surface px-2 py-2 text-left font-semibold">
              Category
            </th>
            {personas.map((p) => (
            ...
            <Fragment key={group}>
              ...
              {FUTURE_CATEGORIES.filter((c) => c.group === group).map((cat) => (
                <tr key={cat.key} className="border-b border-border last:border-0">
                  <td className="sticky left-0 z-10 w-28 min-w-[110px] max-w-[110px] break-words bg-surface px-2 py-1.5 text-[11px] text-muted">
