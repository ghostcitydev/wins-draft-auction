...
import type { QbStatRow } from "@/app/api/qb-stats/route";

export function useQbStats(pollMs = 60000) {
  const [qbs, setQbs] = useState<QbStatRow[] | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch("/api/qb-stats", { cache: "no-store" });
  ...
  return { qbs, week, loading, error, reload: load };
}
